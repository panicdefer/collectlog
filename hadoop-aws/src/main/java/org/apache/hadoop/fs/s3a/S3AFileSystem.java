...
public class S3AFileSystem extends FileSystem {
	...
	public void initialize(URI name, Configuration conf) throws IOException {
		...
		awsConf.setProtocol(
				conf.getBoolean(SECURE_CONNECTIONS, DEFAULT_SECURE_CONNECTIONS) ? Protocol.HTTPS : Protocol.HTTP);
		awsConf.setMaxErrorRetry(conf.getInt(MAX_ERROR_RETRIES, DEFAULT_MAX_ERROR_RETRIES));
		awsConf.setSocketTimeout(conf.getInt(SOCKET_TIMEOUT, DEFAULT_SOCKET_TIMEOUT));
		s3 = new AmazonS3Client(credentials, awsConf);
		LOG.info("===========================================================");
		// 设置REGION
		String regionName = PropertyUtils.get("region").toString().trim();
		String nation = PropertyUtils.get("nation").toString().trim();
	...
	public FSDataOutputStream create(Path f, FsPermission permission, boolean overwrite, int bufferSize,
		...
		return new FSDataOutputStream(new S3AOutputStream(getConf(), s3, this, bucket, key, progress, cannedACL,
				statistics, serverSideEncryptionAlgorithm), null);
	}
	...
	public FSDataOutputStream append(Path f, int bufferSize, Progressable progress) throws IOException {
		 throw new IOException("Not supported");
//		final FileStatus fileStatus = getFileStatus(f);
//		OutputStream out = new S3AOutputStream(getConf(), s3, this, bucket, pathToKey(f), progress, cannedACL,
//				statistics, serverSideEncryptionAlgorithm);
//		return new FSDataOutputStream(out, statistics,fileStatus.getLen());
	}
	 ...
	 * Fails if src is a file and dst is a directory. Fails if src is a
	 * directory and dst is a file. Fails if the parent of dst does not exist or
	 * is a file. Fails if dst is a directory that is not empty.
	 *
	 * @param src
	 *            path to be renamed
	 * @param dst
	...
	public boolean delete(Path f, boolean recursive) throws IOException {
		...
		if (status.isDirectory()) {
			...
			if (status.isEmptyDirectory()) {
				...
				// Hopefully not setting a delimiter will cause this to find
				// everything
		...
		if (fileStatus.isDirectory()) {
			...
			while (true) {
				for (S3ObjectSummary summary : objects.getObjectSummaries()) {
					...
					// Skip over keys that are ourselves and old S3N _$folder$
					// files
					...
					if (objectRepresentsDirectory(summary.getKey(), summary.getSize())) {
						...
						result.add(
								new S3AFileStatus(summary.getSize(), dateToLong(summary.getLastModified()), keyPath));
						if (LOG.isDebugEnabled()) {
	 ...
	 *
	 * This version doesn't need to create a temporary file to calculate the
	 * md5. Sadly this doesn't seem to be used by the shell cp :(
	 *
