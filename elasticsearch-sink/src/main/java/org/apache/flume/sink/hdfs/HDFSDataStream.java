...
public class HDFSDataStream extends AbstractHDFSWriter {
  ...
  protected void doOpen(Configuration conf, Path dstPath, FileSystem hdfs) throws IOException {
    ...
    // ======================modify yangjian  s3 append
    if (conf.getBoolean("hdfs.append.support", false) == true && hdfs.isFile(dstPath)) {
    
   //if (hdfs.isFile(dstPath)) {
  	logger.info("==========================================================================");
  	 logger.info("hdf append support file ");
      outStream = hdfs.append(dstPath);
      appending = true;
    } else {
       logger.info("create new fie");
    ...
    if (appending) {
    	...
    	logger.info("appending  is staring ------------------ afterReopen() ------------------");
      serializer.afterReopen();
    } else {
    	logger.info("appending  is staring ------------------ aftercreatte ------------------");
